...
export default function CheckoutStep({ orderData, clientSecret, onBack }: CheckoutStepProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const returnUrl = `${window.location.origin}/success?email=${encodeURIComponent(email)}&recipient=${encodeURIComponent(orderData.recipientName)}&occasion=${encodeURIComponent(orderData.occasion)}`;
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden step-animate">
      {/* Header */}
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-900">
                {orderData.style} ({orderData.vocalPreference})
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">For:</span>
              <span className="font-medium text-gray-900">
                {orderData.recipientName} ({orderData.occasion})
              </span>
            </div>
            <div className="flex justify-between">
      ...
      <div className="px-6 pb-5 flex items-center justify-center gap-2">
        {["VISA", "MC", "AMEX", "APPLE"].map((card) => (
          <div
            key={card}
            className="h-6 px-2 bg-white border border-gray-200 rounded text-[9px] font-bold text-gray-500 flex items-center"
          >
            {card}
          </div>
        ))}
