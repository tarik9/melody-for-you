...
function VideoCard({ src, poster, label }: VideoItem) {
  ...
  return (
    ...
    >
      ...
      >
        <div className="w-14 h-14 bg-black/40 backdrop-blur-sm rounded-full flex items-center justify-center group-hover:bg-black/60 transition-colors">
          {playing ? (
    ...
    <section className="py-20 bg-white">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 max-w-2xl mx-auto">
          {videos.map((v) => (
            <VideoCard key={v.src} src={v.src} poster={v.poster} label={v.label} />
          ))}
        </div>
      </div>
    </section>
